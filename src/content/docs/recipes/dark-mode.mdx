...
- Includes minimal [`<select>`](#theme-select-component) based [web component](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) (which isn't necesary to use the script in `ThemeManager.astro`)
...
This `.astro` component consists of two `<script>`s. The first is an inline `<script>` that accepts the `defaultTheme` prop and will live in the `<head>` of your layout or pages, it is responsible for ensuring there is no [FOUC](https://en.wikipedia.org/wiki/Flash_of_unstyled_content), creating the `window.theme` client side API, and dispatching a custom event `theme-changed` whenever the theme changes. The second script is not inline and adds an event listener for `astro:after-swap` in order to make this work with [View Transitions](https://docs.astro.build/en/guides/view-transitions/).

The first script is an [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) and it checks if `window.theme` already exists before executing. This prevents the global scope from being polluted, and ensures we don't see any `Identifier has already been declared` errors. The second script is specfically not inline, so that we don't have to worry about the potential for redudant event listeners. 
