...
This `.astro` component consists of an inline `<script>` that accepts the `defaultTheme` prop and will live in the `<head>` of your layout or pages, it is responsible for ensuring there is no [FOUC](https://en.wikipedia.org/wiki/Flash_of_unstyled_content), creating the `window.theme` client side API, and dispatching a custom event `theme-changed` whenever the theme changes.

The script is an [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) and it checks if `window.theme` already exists before executing. This prevents the global scope from being polluted, and ensures we don't see any `Identifier has already been declared` errors.
