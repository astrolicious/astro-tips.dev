...
- Compatible with [View Transitions](https://docs.astro.build/en/guides/view-transitions/)
...
- [Progressive Enhancement](https://en.wikipedia.org/wiki/Progressive_enhancement) meaning when JS is disabled it won't display a broken button or disrupt default styles
...
This script will live in the `<head>` of your layout or pages, it is responsible for ensuring there is no [FOUC](https://en.wikipedia.org/wiki/Flash_of_unstyled_content), creating the developer facing `window.theme` client side API, and dispatching a custom event `theme-changed` whenever the theme changes.
...
1. The first part of our script is `window.theme` and this starts off with getting the defaultTheme prop and creating our store. We need to check if `localStorage` is available to us, because it isn't available everywhere, and make sure we degrade gracefully when it isn't.

2. Now we can get into 
