...
- Compatible with Astro's [View Transitions](https://docs.astro.build/en/guides/view-transitions/) polyfill (renamed to `<ClientRouter/>` in v5)
...
The second script is not inline and adds an event listener for `astro:after-swap` to make this work with Astro's [View Transitions](https://docs.astro.build/en/guides/view-transitions/) polyfill (Renamed to `<ClientRouter/>` in v5).
...
What would a recipe's style section be if it didn't mention [Tailwind CSS](https://tailwindcss.com/), especially when setting it up is easy as this:
...
<Tabs>
  <TabItem label="TypeScript">
    ...
    ```
      ...
      <TabItem label="ESLint Flat config">
    ```js title="eslint.config.js" ins={3-9}
  ...
  <TabItem label="ESLint RC config">
