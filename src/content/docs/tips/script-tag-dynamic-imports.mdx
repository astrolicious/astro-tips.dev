...
Scripts with no attributes in Astro are automatically processed into [`type="module"`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules) and all hoisted into one `.js` file that has static imports to other `.js` files. It is possible to use [dynamic module loading](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Modules#dynamic_module_loading) instead of the default static imports, in order to gain more granular control over if/when the imported code is actually imported (downloaded).
